# 京都ラボ（kyotolab.github.io）

京都ラボの公式サイト。GitHub Pages が `main` をそのまま配信している（ビルドなし、`.nojekyll`）。

**このリポジトリは公開されている。** ページだけでなく、コミットメッセージ、PR本文、このファイルも誰でも読める。

## 事業

- 屋号は京都ラボ（Kyoto Lab）。個人事業として開業準備中。代表は二ノ宮 大知。
- 京都の店のインバウンド対応を、AIとITで軽くする。柱は4つ: アプリの開発と提供、京都の日常を書く（地域メディア）、店舗へのAI導入、店舗の導入支援。
- 考え方は「観測して、実装して、現場に戻す」。トップでは 観測する／実装する／居続ける の3つで書いている。
- 5年の目標は、京都のためのアプリ30種と、京都についての記事100本。
- SNS: Instagram（`kyotolab923`）、note、Medium。リンクは日本語トップ、英語トップ、プロフィールにある。

## ファイル構成

| パス | 中身 |
|---|---|
| `index.html` | 日本語トップ。背景／めざすこと／名所攻略／京都の日常／製品・サービス／事業概要／お問い合わせ |
| `en/index.html` | 英語トップ。日本語トップと同じ構成にする |
| `profile/` | 代表プロフィール。`noindex, nofollow` で検索に出さない |
| `articles/<slug>/index.html` | 記事。1記事1ファイルで、CSSもファイル内に書く |
| `spots/<slug>/index.html` | 名所攻略のページ（日本語）。行く時刻、見るところ、回り方。トップの名所カードはイラストと名前だけで、ここへリンクする。英語の解説（`articles/<slug>/`）があればここからリンクする |
| `en/spots/<slug>/index.html` | 名所攻略の英語版。日本語の `spots/` と同じ事実と節。英語トップの名所カードからリンクする |
| `tools/menu/` | デモ: 多言語お品書き（QRから4言語、アレルギーで絞り込み） |
| `tools/sento/` | デモ: 銭湯の入り方を9手順・4言語で案内 |
| `assets/` | 画像。`<名前>.jpg`（1600px幅）と、カード用の `<名前>@700.jpg` |
| `console/` | 記事管理と Medium 投稿のツールの雛形（PyQt6）。中身はまだない。Medium の API は使えないので（下の「SNS」）、Medium への投稿はここではできない |
| `social/instagram/<slug>.txt` | Instagram の投稿文。`main` に入ると自動で投稿される |
| `social/note/<slug>.txt` | note に手で貼る本文 |
| `social/instagram.py` | Instagram の投稿文の確認、投稿、トークンの更新 |
| `.github/workflows/` | Instagram への投稿（`instagram.yml`）と、トークンの更新（`instagram-token.yml`） |

記事の追加は `/add-article`、公開前の確認は `/site-check` のスキルを使う。

## 書くときのルール

- 代表の経歴は、サイトにすでに載っている範囲だけを使う: 京都市下京区生まれ、24歳まで京都、6年離れて30歳で戻った、立命館大学・同大学院（応用化学）、AI・IT分野。
- それ以外の個人情報は、本人がその場で指示しない限り書かない。勤務先や職歴の細部、働き方、稼働時間、区より細かい住所、通っていた店の名前などが該当する。ページ、コミット、PR、このファイルのどれにも書かない。
- 一人で運営していることは「一人で運営する」「限られた稼働時間」と書く（2026-09-22 に決めた）。
- 日本語ページの数量はアラビア数字で書く（6年、30種、100本）。
- 事実は確かめてから書く。営業時間、料金、規則のように変わるものには、公式情報を確認するよう注記する。
- 画像はオリジナルのイラスト。人が写り込むものは使わない（使うなら写らないように切り抜く）。
- 日本語トップを変えたら、`en/index.html` も同じ構成と同じ事実に揃える。

## デザイン

- 白地のゴシック。色は各ページ冒頭の `:root` 変数を使い、直接色を書かない。
  - 青 `--accent`（#0066CC）は、リンクやボタンなど触れるものだけに使う。
  - 朱 `--gold`（#A33A2C）は、ラベルと通し番号だけに使う。
- フォントはシステムフォント（ヒラギノ、游ゴシック）。外部のフォントやスクリプトは読み込まない。
- 和文の見出しは `font-feature-settings:"palt"` で詰める。`letter-spacing` では詰めない。
- 幅375pxのスマホで見出しや表が崩れないこと。
- 画像はJPEGのファイルとして `assets/` か記事のフォルダに置く。base64 で HTML に埋め込まない。
- ページには `description`、`canonical`、`og:title`、`og:description`、`og:image` を入れる。英語のページは `<html lang="en">` にする。

## お問い合わせフォーム

送信先は Formspree で、`index.html` と `en/index.html` の `<form action>` に書いてある。JSが動けばページ内で送信して必須項目を検証する。JSがなくても通常のPOSTで届く。

## SNS

記事はこのサイトが原本で、SNS にはそこから広げる。手順は `/add-article` の「SNS に広げる」にある。

SNS の目的は集客。主な相手は日本人より外国人観光客で、Instagram でファンを増やし、英語の記事を経て Medium（と note）の有料記事へ誘導する（2026-09-25 にオーナーが決めた）。そのため Instagram の投稿文は英語を先に書き、プロフィールのリンクは英語トップに向ける。

- Instagram: `social/instagram/` に投稿文を足した PR が `main` に入ると、GitHub Actions が投稿する。投稿は取り消せない。
  - GitHub の Secrets に `IG_ACCESS_TOKEN`（Instagram ログインの長期トークン）を入れる。トークンは60日で切れるので、`instagram-token.yml` が毎月1日と15日に期限を延ばす。
  - 延ばしたときに新しい文字列が返ってきたら、`SECRETS_PAT`（このリポジトリの Secrets を書き換えられる fine-grained token）で保存する。`SECRETS_PAT` がなければワークフローが失敗するので、トークンを取り直す。
  - 手で動かすときは、Actions の「Instagram」から Run workflow。既定では投稿の直前まで確かめるだけで、投稿しない。パスを空にすると、トークンが使えるかだけを見る。
  - 公開リポジトリなので、Actions のログは誰でも読める。トークンをログやファイルに出さない。
  - 60日のあいだコミットがないと、GitHub が定期実行を止める。止まったら Actions の画面で有効に戻す。
- Medium: API は使えない（2025-01-01 から新しいトークンを発行していない）。サイトに出た記事の URL を https://medium.com/p/import に貼って取り込む。canonical はサイトに向く。
- note: 公式の API がない。`social/note/<slug>.txt` の本文を手で貼る。非公式の API や自動操作は使わない。

## 作業の進め方

- 1つの変更を1コミットにする。件名は英語の命令形で、何をどう変えたかを具体的に書く（例: `Change the 背景 headline to 六年離れて、京都を学び直した。`）。
- クラウドのセッションは `claude/*` ブランチに push して、ドラフトPRを出す。`main` への取り込みはオーナーが行う。
- `main` の履歴を書き換えない（rebase や force push をしない）。オーナーが指示したときだけ行う。

## 未完了（2026-09-24 時点。片づいたら消す）

- `en/index.html` が日本語トップとずれている。「めざすこと」の節がない。事業の柱が日本語の4本（アプリ、日常を書く、店舗へのAI導入、導入支援）ではなく、古い3枚のまま。事業概要の What we do に店舗へのAI導入がない。
- トップで「公開準備中」になっている記事が2本ある: 「上ル・下ル」の住所と地図アプリの話、銭湯の番台でのやりとりの話。どちらも 2026.10 と表示している。
- `articles/heian-jingu/` と `articles/kamogawa/` は画像を base64 で埋め込んでいる。記事のフォルダに JPEG として出す。
- `tools/menu/` と `tools/sento/` には `og:image` がない。
- Instagram の自動投稿は `IG_ACCESS_TOKEN` を登録して動作確認済み（2026-09-25）。`SECRETS_PAT` はまだ未登録で、入れるまでは60日ごと（次は2026年11月下旬ごろ）にトークンを手で取り直す必要がある。
